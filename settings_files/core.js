	...
	, ajs = jsCore.extend(global["ajs"] || {}, {
		...
		, stacktrace: function (){
			...
			try { halt(); } catch( error ){
				ajs.each(error.stack.split('\n').slice(2), function (row, i){
					var match = row.match(/at\s+([^\s]+)\s+\((.+):(\d+):/);
					...
					if( match != null ){
						stack.push((i ? '  ' : '') + '"'+match[1]+'": line '+match[3]+' in '+match[2]);
					}
				});
			}
...
// ./data/common/js/jquery/jquery.fn.ext.js start
...
(function() {
	 ...
	 **/
	$.fn.toggleClassSiblings = function (name, selector) {
		if (name)
			this.addClass(name).siblings(selector).removeClass(name);
		return this;
	};
}());

window.jsLoader && jsLoader.loaded('{jquery}jquery.fn.ext', 1);

// ./data/common/js/jquery/jquery.fn.ext.js end
...
// ./data/common/js/jquery/jquery.placeholder.js start
...
// ./data/common/js/jquery/jquery.placeholder.js end
...
(function (window, document, jQuery){
		...
		jQuery.getCSS = function (url, options, callback){
			...
			if( typeof url !== 'string' ){
				var i = 0, c = url.length, t = 0, func = function (){
					if( ++t == c ) callback();
				};
			...
			if( document.createStyleSheet ){
				...
				if( options.check ){
					// @todo: Сделал на быструю руку -- т.е. очень плохо, нужно переделывать весь этот плагин
					var pidFail, pidDone = setInterval(function (){
						if( options.check() ){
							clearTimeout(pidFail);
							clearInterval(pidDone);
							callback(link, "success");
						}
					}, 30);

					pidFail = setTimeout(function (){
						callback(link, "error");
						clearInterval(pidDone);
					}, 10000);
				}
			...
			if (!options.url && options.matchContains ) {
				...
				for (k in data ) {
					...
					if (k.length > 0) {
						c = data[k];
						i = 0;
						for( var len = c.length, x ; i < len ; i++ ) {
							x = c[i];
		...
		if( matches ) {
			...
			if( name ) {
				if( !(result = regexp.test(name)) ) {
					parts.push.apply(parts, name.split(_spliter));
				}
			}

			if( !result && email ) {
				if( !(result = regexp.test(email)) ) {
					email = email.split(_email_spliter);
					parts.push.apply(parts, email[0].split(_spliter));
			...
			if( !result ) {
				for( index = parts.length ; index-- ; ) {
					if( result = regexp.test(parts[index]) ) {
						break;
					}
				}
			}
...
//				var top = this.abs ? window.scrollTop() : 0;
				...
				if( !d )
				{
					...
					if (this.$wrapper) {
						if ( !this.styleOverflow ) this.styleOverflow = this.$wrapper.css('overflow');
...
define('profiler/profiler', function() {
	...
	(function (){
		...
		/** @namespace window.performance.mozNow */
		/** @namespace window.performance.msNow */
		/** @namespace window.performance.oNow */
		/** @namespace window.performance.webkitNow */
		...
		*/
		...
		var Profiler = {
			...
			spy: function (name, object, methodName){
				if(disabled) {
					return;
				}

				if( methodName instanceof Array ){
					var i = methodName.length;
					while( i-- ){
						Profiler.spy(name, object, methodName[i]);
					}
				}
				else if( methodName ){
					var origin = object[methodName];

					object[methodName] = function (){
						var label = name+'.fn.'+methodName;
					...
					for( methodName in object ){
						if( object.hasOwnProperty(methodName) ){
							if( typeof object[methodName] === 'function' ){
								Profiler.spy(name, object, methodName);
							}
						}
					}
				}
		...
		Notify.collectorErrorAlert = function (collector) {
			if (store.enabled) {
				...
				var helpers_data = {
					...
					noauth: {
						name: 'attachPowerPoint'
					}
				}, type;

				$.each(helpers_data, function (k, v) {
					helpers_data[k].index = mailru['HelperIndex'][v.name];
					helpers_data[k].timestamp = mailru['HelperTimestamp'][helpers_data[k].index];
				});
				...
				if (type) {
					var helper_data = helpers_data[type];
					if (helper_data) {
						if (helper_data.timestamp < 1356984000) {
							mailru.HelperStatus &= ~(1 << helper_data.index);
						}
						Notify.collectorErrorAlertByType(type, helper_data, collector);
					}
				}
			}
		};

		Notify.collectorErrorAlertByType = function (type, helper_data, collector) {

			var email = mailru.useremail,

				notify_id = 'collector-' + type,

				old_store_go_to_setting_key = 'collector.' + type + '.alert.go_to_setting',
				old_store_last_hide_key = 'collector.' + type + '.alert.last_hide',
				old_store_count_key = 'collector.' + type + '.alert.count',
				old_store_hide_key = 'collector.' + type + '.alert.hide',

				store_go_to_setting_key = email + '.' + old_store_go_to_setting_key,
				store_count_key = email + '.' + old_store_last_hide_key,
				...
				ts_go_to_setting = store.get(store_go_to_setting_key) || store.get(old_store_go_to_setting_key) || 0,
				ts_last_hide = Math.max(store.get(old_store_last_hide_key) || 0, helper_data.timestamp),
				count_hide = store.get(store_count_key) || store.get(old_store_count_key) || 0,
				hide = store.get(old_store_hide_key) || mailru.Balloon.isClosed(helper_data.index)
			;
			...
			if (!hide) {
				...
				if (type == 'noauth') {
					...
					new Image().src = '//rs.' + mailru.SingleDomainName + '/d1559979.gif?' + Math.random();
				} else if (type == 'blocked') {
					new Image().src = '//rs.' + mailru.SingleDomainName + '/d1547999.gif?' + Math.random();
					new Image().src = '//rs.' + mailru.SingleDomainName + '/d1559981.gif?' + Math.random();
				}
				...
				$container.delegate(
					...
					if ($target.hasClass('js-change-password')) {
									...
									if (response.isOk()) {
										mailru.Ajax({
											...
											complete: function () {
												...
												new Image().src = '//rs.' + mailru.SingleDomainName + '/sb1559979.gif?' + Math.random();
											}
										});
						...
						new Image().src = '//rs.' + mailru.SingleDomainName + '/sb1559981.gif?' + Math.random();
						...
						if ($target.hasClass('js-remember-leter')) {
							...
							LayerManager.saveHelper(helper_data.name);
						...
						} else if ($target.hasClass('js-dont-show') || $target.hasClass('js-close')) {

							mailru.Balloon.makeAsClosed(helper_data.index);
...
(function($) {
		...
		.statics({
			...
			noAuth: function (name, vars) {
				...
				if( mailru.phAuthFormEnabled ) {//MAIL-15077
				...
				$R('{plugins}' + 'Layer', function () {
					Layer.get(name, function (layer) {
						...
						var $login = $('.js-field-login', layer.$div);
						...
						layer.one('show', function () {
							...
							$form.bind('submit.layer', function () {
								...
								$('.js-field-page', this).val(defaultUrl);
							});
						});
			...
			changeAccount: function (name, vars) {
				...
				$R('{plugins}' + 'Layer', function () {
					Layer.get(name, function (layer) {
						...
						$('.js-field-login', layer.$div).val(mailru.useremail);
						$('.js-field-page', layer.$div).val(currentUrl);
			...
			listUnsubscribe: function(name, options) {
				$R('{plugins}' + 'Layer', function () {
					Layer.get(name, function (layer) {
						var from = ajs.Html.unescape(options.from);

						layer.$div.find('.js-from')
							.text(from || '')
						;
			...
			collector_add: function (name) {
				$R('{plugins}' + 'Layer', function () {
					Layer.get(name, function(layer) {
						var _$ = function(name) {
							return $('.js-popup__collector__add_' + name, layer.$div);
						};
						...
						var collector = {
							add: function(data, callback) {
								mailru.API.post('collectors/add',
								{
									...
									if (response.isOk()) {
										mailru.Notify.add('ok', {
											text: Lang.get('Collector').add.success
										});
										...
										else if (error.is('over_limit')) {
											this.notify('limit');
										}
										else if (error.is('required')) {
											this.notify('required');
										}
							...
							error: function(response) {
								...
								return {
									is: function(error) {
										...
										if (error == 'internal' && response.status !== 400 ||
											!Object.isObject(data) || data == error)
										{
											return true;
										}
							...
							notify: function(error) {
								var message = Lang.get('Collector').edit.error[error];
							...
							value: function(value) {
								return _$('input_' + value).val();
							},

							submit: function(state) {
								// Disable submit button
								_$('submit')
									.toggleClass('form__button_disabled', state);

								_$('loading')
									.toggleClass('dn', !state);
							...
							domains: function() {
								window.__MAIL_DOMAINS = function(response) {
									if (Object.isObject(response)) {
										var domains ='';

										Object.forEach(response.Domains, function(data, domain) {
											domains += '<option value="' + domain + '">@' + domain + '</option>'
										});

										if (domains) {
											_$('input_domain').html(domains);
										}

										mailru.collector_domains = response.Domains;
									}
								};

								$.ajax({
									url: 'https://swa.mail.ru/domains',
									data: {
										v2: 1,
										JSONP_call: '__MAIL_DOMAINS'
									},
									dataType: 'script'
								});
							},
						...
						if (!mailru.collector_domains) {
							collector.domains();
						}

						// Reset message errors
						collector.notify(null);

						layer.$div
							.unbind('.layer-collector__add')
							.delegate('.popup__collector__add_domains__list', 'click.layer-collector__add', function(event) {
								var domain = $(event.currentTarget).data('domain');

								_$('select').display(domain != 'other');

								_$('input_domain').val(domain)
									.change();
							})
						;

						layer.func = function(status) {
							var email    = collector.value('login'),
								password = collector.value('password'),
								domain   = collector.value('domain');

							if (!email) {
								return collector.notify('login');
							}
							else if (!password) {
								return collector.notify('password');
							}
							else if (_$('select').is(':hidden')) {
								if (domain = email.match(/@(.{4,})$/)) {
									domain = domain[1];
								}
								else {
									return collector.notify('domain');
								}
							}
							else {
								email += '@' + domain;
							...
							var type = collector.imap[domain];

							if (type) {
								auth_data.type = type;

								collector.add(auth_data, function() {
									collector.submit(false);
								});
							}
							else {
								...
								function(response) {
									if (response.isOk()) {
										var type = response.getBody().imap == true ? 'imap' : 'pop3';

										// Cache data to avoid the non unique requests
										collector.imap[domain] = auth_data.type = type;
					...
					{
						className: 'popup__collector__add'
					});
				});
			},
			...
			increaseSpace: function() {
				$R([
					...
					Layer.get('increaseSpace', function(layer) {
						var $template = layer.$div.find('#js-increaseSpace'),
							templateId = '#js-increaseSpace-tmpl';
						...
						function render(data) {
							...
							$template.tpl(templateId, data);
						}

						layer.$div
							.undelegate('.js-refresh-captcha', 'click')
							.delegate('.js-refresh-captcha', 'click', function() {
								layer.$div.find('#captcha_frame').attr('src', mailru.captchaURL1 + '?' + Math.random());
								return false;
							});

						layer.$div
							.undelegate('.confirm-cancel', 'click')
							.delegate('.confirm-cancel', 'click', function() {
								layer.hide();
								return false;
							});

						layer.$div
							.undelegate('.confirm-ok', 'click')
							.delegate('.confirm-ok', 'click', function() {
								mailru.API.post(
									'user/box/increase',
									{
										capcha: layer.$div.find('#captcha').val()
									},
									function(response) {
										var body = response.getBody(),
											$error = layer.$div.find('.js-formError');
										...
										if (response.isOk()) {
											...
											if (body.capcha && body.capcha.error == 'invalid') {
												$error
													.text('Неверно указан код')
													.show();
											}
											else if (body.capcha && body.capcha.error == 'required') {
												$error
													.text('Укажите код с картинки')
													.show();
											}
											else if (body == 'not_needed') {
												render({
													error: {
														msg: 'Размер вашего ящика на данный момент оптимален'
													}
												});
											}
											else if (body == 'too_big') {
												render({
													error: {
														msg: 'Ваш почтовый ящик достиг максимального размера. К сожалению, увеличить размер почтового ящика невозможно'
													}
												});
											}
											else {
												$error
													.text('Неизвестная ошибка, повторите запрос позднее')
													.show();
											}
										}
									}
...
// GLOBALIZATION
	...
	(function (win, Utils){
		...
		// MAIL-16654, MAIL-16684, MAIL-16792
		Utils.spinner = new function() {
			var key = mailru.useremail + '|collector.spinner';
			...
			return {
				show: function() {
					...
					if (folder) {
						// 4. Show the spinner only once (one cycle collector)
						if (this.get(collector.id)) {
							return false;
						}
						else if (!collector.error && collector.state == 'work') {
							this.set(collector.id);
							return false;
						}
						else if (!folder.Messages) {
							// 5. There is only one collector and one folder (initially)
							...
							// 6. System folder <Inbox> initially is not empty
				...
				'set': function(id) {
					if (id) {
						try {
							var spinner = store.get(key) || {};
							spinner[id] = true;
							store.set(key, spinner);
						}
						catch(e) {}
					}
				},
...
(function (){
		...
		var Folder	= mailru.OldModel.extend({
			...
			clear: function (){
				...
				mailru.log('folder.clear', this.id);
				...
				if( !this.__clearDefer ){
					...
					if( !this.id ){
						df.reject();
						mailru.log('folder.clear.fail', this.id);
					}
					else {
						mailru.Ajax({
							url: "/cgi-bin/clearfolder",
							...
							complete: function (R){
								...
								if( R.isOK() ){
									...
									mailru.log('folder.clear.done', this.id);
								}
								else {
									mailru.log('folder.clear.fail', this.id);
								}
	...
	jsClass
		...
		recalcCounters: function (){
			...
			Object.forEach(this.Map, function (folder){
				if (!folder.isBulk() && !folder.isTrash()) {
			...
			if( !force && (mode != 'new' && mode != 'drafts') && Msg && ('let_body_re' in Msg) ){
				...
				jsCore.wait(this.id+'.init', function (){
					...
					if (mailru.v2) {
						...
						if (GET.To) {
							extra_fields.To = GET.To;
						} else if (GET.mailto) {
							var mailto = GET.mailto.match(/(^mailto:)(.*)/);
							extra_fields.To = mailto && mailto[2];
						} else if (GET.CC) {
							extra_fields.CC = GET.CC;
						} else if (GET.BCC) {
							extra_fields.BCC = GET.BCC;
						}
					}
...
/** @namespace	this.isNOP() */
	...
	function doResult (response, opts, xhr, s, status, error, old_answer) {
		...
		var convert_response = response;

		if (s == 'success' && opts.convertResultToOldApi && !old_answer) {
			if( mailru.v2 && response && response.build ){
				mailru.needReloadPage('build', response.build);
			}

			convert_response = mailru.Utils.AjaxConverter.convert(response, opts);
		}

		var result = new mailru.Ajax.Result(convert_response, opts, xhr, s, error);
		...
		if( mailru['UseTokenAPIV1'] && result.isBadToken() && opts.tokenRetry-- > 0 ){
			...
			$.ajax({
				url: 'https://'+ mailru['UseSDC'] +'/sdc?JSONP_call=?&from=' + ajs.encode(ajs.Router.parseURL(jsHistory.get()).url),
				type: 'get',
				dataType: 'jsonp',
				timeout: 3000,
				error: function (){ doSDC(false); },
				success: function (state){ doSDC(state); }
			});
		}
		else {
		...
		// MAIL-17187
		if (s === 'success' && !old_answer && opts.isAPIV1 && response && response.status) {
			var api_key_re = /^\/api\/v\d+\/([^\?]+).*/;
			if (api_key_re.test(opts.url)) {
				var api_key = opts.url.replace(api_key_re, '$1').replace(/\//g, '_'), radar_parts;
				if (response.status == 403) {
					radar_parts = [
						  api_key + '_' + response.status + '=1'
						, api_key + '_' + response.status + '_' + response.body + '=1'
						, response.status + '=1'
					];
				} else if (api_key !== 'messages_status' || response.status != 304) {
					radar_parts = [
						  api_key + '_' + response.status + '=1'
						, response.status + '=1'
					];
				...
				if (radar_parts) {
					mailru.radar('APIS', radar_parts.join('&'));
				}
			}
		}
	}
							...
							$(window).one('authUser.success', function (name, vars, evt, response) {
								if (response.status == 'ok') {
									...
									if (Array.isArray(list)) {
										...
										else if( mailru.phAuthFormEnabled ) {//MAIL-16642
											vars.formType = "relogin";
										}
									}
									else if( mailru.phAuthFormEnabled ) {//MAIL-16642
										vars.formType = "relogin";
...
(function (){
		...
		var Utils = {
			...
			isGraphicPreview: function (file) {
				...
				return (mailru['IsMRVGraphicPreviewEnabled'] && (ext == "psd" || ext == "tga" || ext == "ai" || ext == "tif" || ext == "tiff" || ext == "eps"));
			},
			...
			isOfficePreview: function(file) {
				...
				return (
					   (mailru['IsCorpUser'])
					&& (mailru['IsMRVMSDocPreviewEnabled'] || mailru['IsMRVMSPptPreviewEnabled'] || mailru['IsMRVMSExcelPreviewEnabled'])
...
//			Timeout.set(this.classUniqId+'cleanMemory', $DS(this, '_cleanMemory'), 1000);
		...
		upd: function (id, data, safe, fields){
			var M = this.get(id), radar = Math.random() < .2, extra_data = {};
			...
			if (M) {
				...
				if (fields) {
					...
					var fields_to_update = ['Flagged', 'Forward', 'Reply', 'Unread', 'NextId', 'PrevId'];
					Object.forEach(data, function (value, name) {
						if (Array.indexOf(fields_to_update, name) !== -1) {
		...
		load: function (params, preload, read){
			...
			*/
			...
			if( !(Msg._loading || Msg._loaded) || (!preload && (preload !== void 0)) ){
				mailru.log('message.load', {
					id: Msg.Id,
					force: (!preload && (preload !== void 0)),
					preload: preload
				});
				...
				var ajaxData = {
					...
					, data:		ajs.extend({
								...
								}, params, { read: read ? params.id : 0, mark_read: !!read })
					, storeMe:  'readmsg'
			...
			switch( type )
			{
				...
				case 'remove':
				{
					...
					else
					{
						...
						// Remove messages
						if( deleted ){
							if (mailru.isReadMsgPage()) { // MAIL-12195
						...
						if( mailru.isReadMsgPage() ){
							var nId = M[0].get('NextId'), fId = mailru.getFolderId();
	...
	mailru.Messages.find = function (data){
		...
		if( data.id ){
			...
			if( msg && msg.isLoaded() ){
				...
				mailru.Messages.load( data, void 0, data.read );
			}
		}
		else {
			// Find messages
...
(function ($, mailru, undef){
		...
		mailru.needReloadPage = function (key, val){
			/** @namespace document.TS -- head.all.html */
			/** @namespace mailru.KaktamBuildEnabled -- https://jira.mail.ru/browse/MAIL-18164 */

			function _toDate(val){
				return	new Date(val.replace(/_/g, ':'));
			}
			...
			if( mailru.v2 ){
				if( (key == 'build') && val && (mailru.build != val) ){
					var
						  ts = new Date
						, build = val.split('.')
						, newBuild = _toDate( build[1] )
						, oldBuild = _toDate( mailru.build.split('.')[1] )
						, vector = (newBuild > oldBuild) ? 'UP' : 'DOWN'
						, log = {
							  email: mailru.useremail
							, session_start: new Date(document.TS).toISOString()
							, session_end: 0
							, build_old: mailru.build
							, build_new: val
							, build_receive: ts.toISOString()
							, vector: vector
							, page_from: mailru.getPageLabel()
						}
					;

					if( isNaN(newBuild.getTime()) ){
						mailru.radar('deploy_fail', 'upd=1', 1);

						if (mailru.KaktamBuildEnabled) {
							log.deploy_fail = 1;
							mailru.gstat(log);
						}
					...
					mailru.radar('deploy_upd', vector+'=1');
					...
					mailru.needReloadPage	= function (key, val){
						if( key == 'href' ){
							log.page_to = mailru.getPageLabel(val);
						}
						else {
							log.page_to = key;
						}

						mailru.radar('deploy_'+build[0], build[1]+'=1');
						mailru.radar('deploy_switch', (new Date - ts));

						if (mailru.KaktamBuildEnabled) {
							log.session_end = new Date(new Date - document.TS).toISOString();
							log.deploy_switch = 1;
							mailru.gstat(log);
						}
						...
						if( /^(fastanswer|compose|sendmsgok)$/i.test(key) ){
							...
							setTimeout(function (){ location.href = val; }, 50);
							return	true;
						}
	...
	mailru.Utils.AjaxConverter = {
		...
		messageToReadmsg: function (messageJSON, ajaxData) {
			...
			if (messageJSON) {
				...
				if (messageJSON.status == 400) {
					...
					} else if (messageJSON.body == 'nosdc') {
						readmsgJSON = ['AjaxResponse', 'NoSDC'];
					} else {
			...
			if (statusJSON) {
				...
				if (statusJSON.status) {
					...
					if (statusJSON.status == 304) {
						...
						} else if (statusJSON.body == 'nosdc') {
							// MAIL-12297
							checknewJSON.status = 'nosdc';
		...
		statusMessageToOldApi: function (message) {
			...
			if (message.flags) {
				...
				newMessage.ReadSet = message.flags.read_set;
			}
...
//			jsCookie.set('threads', 1, jsCookie.WEEK);
		...
		var Ajax = function (_options) {
			...
			return {
				call: function (params) {
					if (mailru['OfflineMode'] || _options.token || params.method === 'tokens' || !ajs.isset(params.tokens, true) || params.anonymous) {
						this.send(params);
					} else {
						this.token(params);
					}
				...
				send: function (params) {
					...
					if (params.proxy) {
						url = '/api-proxy/' + params.proxy + (params.method || params.url);
					}
		...
		var API = function (_options) {
			...
			_api.call = function (type, method, data, fn, loading, hideAutoErrors, anonymous) {
				if ($.isFunction(data)) {
					fn = data;
					data = undefined;
				}
			...
			$.each(['post', 'get'], function (k, type) {
				_api[type] = function (method, data, fn, loading, hideAutoErrors, anonymous) {
					return _api.call(type, method, data, fn, loading, hideAutoErrors, anonymous);
				}
			});
...
/** @namespace mailru.themes */
	...
	mailru.Themes = {
		...
		, _initElements: function(){
			...
			var $el = this._themeIdEl = $('#themeId');
			this._cssProp = 'content';

			if( $el.css('content') != $el.css('fontFamily') ){
				this._cssProp = ($el.css('content') + '').indexOf(this.getCurrentThemeId()) != -1 ? 'content' : 'fontFamily';
			}
		}
		...
		, isApplyedThemeId: function(id){
			return	(this._themeIdEl.css(this._cssProp) + '').indexOf(id) != -1;
		}
		...
		, switchThemeCss: function(themeId, themeVariant){
			...
			$.getCSS(
				...
				, {
					  noCache: true
					, check: function (){
						return	this.isApplyedThemeId(themeId);
					}.bind(this)
				}
				, function(link){
					if (this.isApplyedThemeId(themeId)){
						if( this.$ThemeCSS[0] ){
							this.$ThemeCSS[0].cssText = '';
							this.$ThemeCSS.remove();
						}
						this.$ThemeCSS = $(link);
						this.redrawThemeLink(themeId);
						this.adaptiveBackground(themeId);
...
/**
	...
	.methods({
		...
		__construct: function() {
			...
			this.$wrapper = $(document.getElementById('ScrollBody') || document.documentElement);
	...
	.methods({
		...
		_update: function () {
			// MAIL-16393
			var scrollTop = $(document.getElementById('ScrollBody') || window).scrollTop();
	...
	.methods({
		...
		__construct: function (type, callback) {
			...
			this.mods = this.$div.attr('class').replace(this.type+'_in', '').split(/\s/);
		},
		...
		toggleMods: function(state) {
			Object.forEach(this.mods, function(mod) {
				this.mainDiv.$innerDiv.toggleClass(mod, state);
				this.$div.toggleClass(mod, !state);
			}, this);
		},
		...
		_toggleClass: function(state) {
			...
			if (Layer.options && (className = Layer.options.className)) {
				this.mainDiv.$innerDiv[state ? 'addClass' : 'removeClass'](className);
			}
		},
					...
					ajs.require('{mailru.ui}'+'mailru.ui.SearchSuggest', function (){
						...
						function onShow(e) {
							var input = e.target.$Input,
								suggestsContainer = e.target.$List;

							if( (suggestsContainer = suggestsContainer[0])
								&& (suggestsContainer = suggestsContainer.parentNode)//THIS IS HACK!
								) {
								suggestsContainer.style.width = input[0].clientWidth + "px";
							}
						}

						(new mailru.ui.SearchSuggest('#adv-search-theme-name', opts).Search.Suggest).addTrigger("suggests:show", onShow);
						(new mailru.ui.SearchSuggest('#adv-search-word-contain', opts).Search.Suggest).addTrigger("suggests:show", onShow);
					});
				...
				if(!suggestOptions.grouping) {
					...
					emails = Array.map(emails, function (str, i) {
						...
						if( nameAndEmail_parts ) {
							...
							email = "";
							if( str.contains("@") ) {
								avatar = mailru.Utils.getAvatarSrc(str, "", 32, null);
							}
						}
						...
						item = new GroupedSuggestItem(
							...
							, name
							, val
							, email && (suggestOptions.searchPeopleOnlyForEmail ? email : (name + " <" + email + ">"))
							, needGroupHeader && groupsTitles["peoples"]
							, i
						);

						if( suggestOptions.searchPeopleOnlyForEmail ) {
							suggest = _HTML_escape_(email ? email : name);
						}
						else {
							suggest = _HTML_escape_(name);
						}
								...
								(
									createTick(name) + " " +
										"<span class='" + suggestOptions.cdSuggestHint + "'>" +
											( email ? createTick(email) : "&nbsp;" ) +
										"</span>"
								) +
								(avatar ?
									"<img alt='' class=\"" + suggestOptions.cnItem + "_people__image\" width=30 height=30 src='" + avatar + "' />"
									:
									""
								) +
							'</div>'
			...
			_initAjsSuggest: function() {
				...
				this.ajsSuggest
					...
					.addTrigger("suggests:afterRebuild", function(e) {
						...
						tmp = typeof suggestOpts.width;

						if( tmp === "string" ) {
							if( suggestOpts.width === "auto" ) {
								if( tmp = input[0] ) {
									suggestOpts.width = Math.max(tmp.clientWidth, 100) + "px";
								}
							}

							tmp = suggestOpts.width;
						}
						else if( tmp === "number" && suggestOpts.width > 0 ) {
							tmp = suggestOpts.width;
						}
						else {
							tmp = Math.max(input.width(), 100);
						}
